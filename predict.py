...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_id", type=str, default=None,
                        help="ID (datetime) of pretrained models to use, '-1' for latest, '-2' for second latest, etc")
    ...
    if not os.path.isfile(f"{model_path}/models.pt"):
        raise Exception(f"<{model_path}/models.pt> does not exist.")

    # Get configs of models
    print(f'Using models from {model_path}')
    ...
    # Check that models is trained on specified dataset
    ...
    load(model, f"{model_path}/models.pt", device=device)
    model.to(device)
    ...
    # Creating a new summary-file each time when new prediction are made with a pre-trained models
    count = 0
