...
class MTAD_GAT(nn.Module):
    ...
    def __init__(
        ...
        #self.lstm = LSTMLayer(3 * n_features, gru_hid_dim)
        ...
        #self.autocorr = AutoCorrelation(True,1,None,dropout,False)
    ...
    def forward(self, x):
        ...
        x = pyt.utils.autocorrelation(x,dim=0)
        x = torch.sqrt(x)
        #x = self.autocorr(x)
        # print('size_x',x.shape,'size_autocorr',x_tmp.shape)
       ...
       #
        ...
        context, attn_scores = self.attention(h_cat)

        #_, h_end = self.gru(h_cat)
        _, h_end = self.gru(context.unsqueeze(1))
        h_end = h_end.view(x.shape[0], -1)   # Hidden state for last timestamp

        #h_end = self.rnn(h_end)
