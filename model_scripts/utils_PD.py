...
def get_data(dataset, max_train_size=None, max_test_size=None,
             normalize=False, spec_res=False, train_start=0, test_start=0, dataset_root=None):
    """
    ...
    #print(os.path.join(prefix, dataset + "_test.pkl"))
...
def load(model, PATH, device="cpu"):
    """
    Loads the models's parameters from the path mentioned
...
def adjust_anomaly_scores(scores, dataset, is_train, lookback):
    ...
    :param lookback: lookback (window size) used in models
    """

    # Remove errors for time steps when transition to new channel (as this will be impossible for models to predict)
